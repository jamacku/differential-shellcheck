...
csgrep --mode=sarif ../bugs.log >> output.sarif 
cat output.sarif
curl -v -X POST \
  -f "https://api.github.com/repos/${GITHUB_REPOSITORY}/code-scanning/sarifs" \
  -H "Authorization: token ${INPUT_TOKEN}" \
  -H "Accept: application/vnd.github.v3+json" \
  -d '{"commit_sha":"'"${INPUT_HEAD}"'","ref":"refs/pull/20/head","sarif":"'$(gzip -c output.sarif | base64 -w0)'","tool_name":"csdiff"}'

echo '{"commit_sha":"'"${INPUT_HEAD}"'","ref":"'"${GITHUB_REF}"'","sarif":"'$(gzip -c output.sarif | base64 -w0)'","tool_name":"csdiff"}'
